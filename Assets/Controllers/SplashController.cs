	...
	private IEnumerator _CheckMaintenance ()
	{
		...
		if (checkRequest.responseHeaders["STATUS"] == HTML.HTTP_200)
		{
			if (checkRequest.text == IN_MAINTENANCE)
			{
				...
				AlertsService.makeAlert("EM MANUTENÇÃO", "O Meu Vivo Museu está em manutenção no momento. Por favor, tente novamente mais tarde.", "Entendi");
		...
		{
			loadingHolder.SetActive(false);
			AlertsService.makeAlert("FALHA NA CONEXÃO", "Houve uma falha na conexão com o Meu Vivo Museu. Por favor, tente novamente mais tarde.", "OK");
			reloadButton.gameObject.SetActive(true);
		}
